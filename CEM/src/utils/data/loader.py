...
def load_dataset(data_file):
    ...
    cache_file = f"{data_dir}/{data_file}"
...
def collate_fn(data):
    def merge(sequences, cs=False):
        lengths = [len(seq) for seq in sequences]
        if cs:
            max_len = max(max(lengths), 3)  # avoid kernel size larger than input size
        else:
            max_len = max(lengths)
        padded_seqs = torch.ones(len(sequences), max_len).long()  ## padding index 1
    ...
    for r in relations:
        pad_batch, _ = merge(item_info[r], cs=True)
...
def prepare_data_seq(batch_size=32, data_file="dataset_preproc_link.p"):

    pairs_tra, pairs_val, pairs_tst, vocab = load_dataset(data_file)
